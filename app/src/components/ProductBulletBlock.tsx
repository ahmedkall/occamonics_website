...
const ProductBulletBlock = ({ heading, subtitle, bullets, callout, imageSrc, imageAlt, reversed = false }: Props) => {
  ...
  const textBlock = (
    ...
    >
      <h3 className="text-[1.35rem] font-semibold leading-[1.1] tracking-[-0.04em] text-[#001f2b]">
        {subtitle}
      </h3>
      <ul className="mt-6 space-y-4">
        {bullets.map((b) => (
          <li key={b} className="flex items-start gap-3 text-[0.88rem] leading-6 text-[#597786]">
            <Check className="mt-0.5 h-4 w-4 flex-none text-[#1fd3ee]" />
            {b}
          </li>
        ))}
      </ul>
...
export default ProductBulletBlock;
